...
(function() {
  ...
  function handleMouseMove(e) {
    ...
    if (element && element !== infoBox && !infoBox.contains(element)) {
      hoveredElement = element;
      hoveredElement.style.outline = '3px solid #2196F3';
      hoveredElement.style.boxShadow = '0 0 10px rgba(33, 150, 243, 0.5)';
    }
  }
  ...
  function handleClick(e) {
    ...
    // Check if click is on info box or its children
    if (e.target === infoBox || infoBox.contains(e.target)) {
      console.log('Click on info box, ignoring');
      return; // Let the click through for buttons
    }
    
    // Prevent default for page clicks
    e.preventDefault();
    e.stopPropagation();
  ...
  function cleanup() {
    ...
    // Remove all event listeners (must match capture flag)
