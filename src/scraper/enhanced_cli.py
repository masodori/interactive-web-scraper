...
def _run_create_enhanced(args: argparse.Namespace = None):
    """Enhanced template creation with all new features"""
    cli = InteractiveCLI()

    try:
        print("\n🚀 Starting Enhanced Template Creation")
        print("=" * 50)
        ...
        try:
            ...
            print("⚠️  Chrome/Chromium browser not found or not properly configured.")
            print(
                "   This is only needed for Selenium engine with interactive selection."
            )
            print(f"   Error: {e}")

            use_requests = cli.get_user_confirmation(
                "Continue with Requests engine (no interactive selection)?",
                default=True,
            )
            if not use_requests:
                return
...
def _run_apply_enhanced(args: argparse.Namespace = None):
    ...
    if args:
        ...
        selected = cli.select_from_list(
            templates, "Select template:", display_func=lambda t: t.name
        )
        ...
        engine_choice = cli.select_from_list(
            ...
            "Select scraping engine:",
            display_func=lambda x: (
                f"{x} - {'JavaScript support' if x != 'requests' else 'Fast, no JS'}"
            ),
        )
        ...
        # Features
        auto_migrate = cli.get_user_confirmation(
            "Auto-migrate template if needed?", default=True
        )
        use_patterns = cli.get_user_confirmation(
    ...
    try:
        ...
        result = scraper.apply_template(
            str(template_path), export_formats=export_formats, auto_migrate=auto_migrate
        )
...
def _run_analyze_site(args: argparse.Namespace = None):
    ...
    try:
        ...
        load_more_patterns = advanced.find_by_text_content(
            "load more", tag="button", fuzzy=True, min_similarity=0.7
        )
        ...
        print(
            f"  • JavaScript frameworks: {', '.join(detected_frameworks) if detected_frameworks else 'None detected'}"
        )
...
def main():
    ...
    try:
        ...
        # Create command (enhanced)
        create_parser = subparsers.add_parser(
            "create", help="Create a new scraping template with advanced features"
        )
        create_parser.set_defaults(func=_run_create_enhanced)

        # Apply command (enhanced)
        apply_parser = subparsers.add_parser(
            "apply", help="Apply template with enhanced features"
        )
        apply_parser.add_argument("template", type=Path, help="Template JSON file")
        apply_parser.add_argument(
            "--engine",
            choices=["selenium", "requests", "playwright"],
            default="selenium",
            help="Scraping engine",
        )
        apply_parser.add_argument(
        ...
        migrate_parser = subparsers.add_parser(
            "migrate", help="Migrate templates to latest version"
        )
        ...
        patterns_parser = subparsers.add_parser(
            "test-patterns", help="Test pattern extraction"
        )
        patterns_parser.add_argument(
            "--text", help="Text to extract from (interactive if not provided)"
        )
        patterns_parser.set_defaults(func=_run_test_patterns)

        # Analyze command (new)
        analyze_parser = subparsers.add_parser(
            "analyze", help="Analyze website for optimal strategy"
        )
